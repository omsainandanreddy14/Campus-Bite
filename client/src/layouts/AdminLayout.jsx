...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { label: 'Canteens & Coupons', path: '/admin/canteens', icon: Store },
      ...
      <header className="h-16 bg-white/80 dark:bg-slate-900/85 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between px-6 z-30 sticky top-0">
        ...
        <div className="flex items-center gap-6">
          <Link to="/admin/dashboard" className="flex items-center gap-2.5 no-underline">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-500/20 text-white font-black text-base">
              A
            </div>
            <span className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-purple-600 to-indigo-500 bg-clip-text text-transparent">
              CampusBite
            </span>
            <span className="text-[10px] font-bold px-2 py-0.5 bg-purple-100 dark:bg-purple-950 text-purple-600 dark:text-purple-400 rounded-md hidden sm:inline-block">
              Super Admin
            </span>
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-purple-50 dark:bg-purple-955/30 border border-purple-200/60 dark:border-purple-900/40 rounded-full text-xs font-semibold text-purple-700 dark:text-purple-400">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <div className="h-8 w-8 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center font-black text-white text-xs shadow-md">
                {user?.name?.[0] || 'A'}
              </div>
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-4 z-50 animate-fade-in space-y-4">
                ...
                <div 
                  ...
                  className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-955 rounded-2xl border border-slate-100 dark:border-slate-850 cursor-pointer hover:border-purple-500/40 transition-all"
                >
                  <div className="h-11 w-11 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center font-black text-white text-base shrink-0 shadow-md">
                    {user?.name?.[0] || 'A'}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-extrabold text-slate-900 dark:text-white truncate">{user?.name}</p>
                    <p className="text-[10px] text-slate-400 truncate">{user?.email}</p>
                    <span className="text-[9px] font-extrabold text-purple-600 dark:text-purple-400 uppercase tracking-wider block mt-0.5">View Admin Profile →</span>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-955/30 transition-all cursor-pointer border-0"
                  >
                    <div className="flex items-center gap-3">
