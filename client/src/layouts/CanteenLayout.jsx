...
const CanteenLayout = () => {
  ...
  const navItems = [
    { label: 'Home Page', path: '/canteen/dashboard', icon: LayoutDashboard },
    { label: 'Add Food Item', path: '/canteen/add-food', icon: PlusCircle },
    { label: 'Food List Menu', path: '/canteen/food-list', icon: List },
    { label: 'Live Orders Queue', path: '/canteen/orders', icon: ClipboardList },
    { label: 'Storefront Profile', path: '/canteen/profile', icon: User },
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-955 flex flex-col transition-colors duration-300">
      ...
      <header className="h-16 bg-white/80 dark:bg-slate-900/85 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between px-6 z-30 sticky top-0">
        ...
        <div className="flex items-center gap-6">
          <Link to="/canteen/dashboard" className="flex items-center gap-2.5 no-underline">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 text-white font-black text-base">
              C
            </div>
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 bg-emerald-100 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 rounded-md hidden sm:inline-block">
              Canteen Owner
            </span>
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {user?.status === 'Suspended' ? (
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-red-50 dark:bg-red-955/30 border border-red-200/60 dark:border-red-900/40 rounded-full text-xs font-bold text-red-600 dark:text-red-400">
              <Store className="h-3.5 w-3.5 text-red-500" />
              <span>Store Suspended</span>
            </div>
          ) : user?.kitchenStatus === 'Closed' ? (
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-amber-50 dark:bg-amber-955/30 border border-amber-200/60 dark:border-amber-900/40 rounded-full text-xs font-bold text-amber-600 dark:text-amber-400">
              <Store className="h-3.5 w-3.5 text-amber-500" />
              <span>Store Closed</span>
            </div>
          ) : (
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-emerald-50 dark:bg-emerald-955/30 border border-emerald-200/60 dark:border-emerald-900/40 rounded-full text-xs font-semibold text-emerald-700 dark:text-emerald-400">
              <Store className="h-3.5 w-3.5 text-emerald-500 animate-pulse" />
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <div className="h-8 w-8 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center font-black text-white text-xs shadow-md">
                {user?.name?.[0] || 'C'}
              </div>
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-4 z-50 animate-fade-in space-y-4">
                ...
                <div 
                  onClick={() => {
                    setProfileDropdownOpen(false);
                    navigate('/canteen/profile');
                  }}
                  className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-955 rounded-2xl border border-slate-100 dark:border-slate-850 cursor-pointer hover:border-emerald-500/40 transition-all"
                >
                  <div className="h-11 w-11 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center font-black text-white text-base shrink-0 shadow-md">
                    {user?.name?.[0] || 'C'}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-extrabold text-slate-900 dark:text-white truncate">{user?.name}</p>
                    <p className="text-[10px] text-slate-400 truncate">{user?.email}</p>
                    <span className="text-[9px] font-extrabold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider block mt-0.5">View Store Profile →</span>
                  </div>
                </div>
